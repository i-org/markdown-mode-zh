;;; markdown-mode-zh.el --- Major mode for Markdown-formatted text -*- lexical-binding: t; -*-
...
(defvar markdown-mode-map
  (let ((map (make-keymap)))
    ...
    (define-key map (kbd "M-<RET>") 'markdown-insert-header-dwim)  ; 换为和orgmode一致的标题快捷键
...
(easy-menu-define markdown-mode-menu markdown-mode-map
  ...
  '("〖码道式〗"
    ...
    "---"
    ("显隐/跳转 文本结构"
     ["显/隐 逐级可见" markdown-cycle (markdown-on-heading-p)]
     ["显/隐 全文可见" markdown-shifttab]
     "---"
     ["显/隐 本地图像" markdown-toggle-inline-images]
     "---"
     ["上一可见标题" markdown-previous-visible-heading]
     ["下一可见标题" markdown-next-visible-heading]
