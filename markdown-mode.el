...
(easy-menu-define markdown-mode-menu markdown-mode-map
  ...
  '("〖码道式〗"
    ...
    ("文件操作"
     ...
     ["实时预览" markdown-live-preview-mode
      :style radio
    ...
    ("段内样式"
     ...
     ["删除线" markdown-insert-strike-through]
     ["行内代码 code" markdown-insert-code]
     ["插入 kbd" markdown-insert-kbd])
     "---"
    ("列表项"
     ["插入 列表项" markdown-insert-list-item]
     ["插入任务列表" markdown-insert-gfm-checkbox
      :keys "C-c C-x ["]
     ["开关/任务列表" markdown-toggle-gfm-checkbox
      :enable (markdown-gfm-task-list-item-at-point)
      :keys "C-c C-d"]
     ["重整 列表编号" markdown-cleanup-list-numbers])
    ("表格"
     ["整行上移" markdown-move-up
      :enable (markdown-table-at-point-p)
      :keys "C-c <up>"]
     ["整行下移" markdown-move-down
      :enable (markdown-table-at-point-p)
      :keys "C-c <down>"]
     ["整列左移" markdown-demote
      :enable (markdown-table-at-point-p)
      :keys "C-c <left>"]
     ["整列右移" markdown-promote
      :enable (markdown-table-at-point-p)
      :keys "C-c <right>"]
     ["插入一行Insert Row" markdown-table-insert-row
      :enable (markdown-table-at-point-p)]
     ["删除一行Delete Row" markdown-table-delete-row
      :enable (markdown-table-at-point-p)]
     ["插入一列" markdown-table-insert-column
      :enable (markdown-table-at-point-p)]
     ["删除一列" markdown-table-delete-column
      :enable (markdown-table-at-point-p)]
     "--"
     ["选定区转为表格" markdown-table-convert-region]
     ["表格排序" markdown-table-sort-lines
      :enable (markdown-table-at-point-p)]
     ["表格行列互换" markdown-table-transpose
      :enable (markdown-table-at-point-p)])
    ("图片 & 连接"
     ...
     ["插入脚注" markdown-insert-footnote
      :keys "C-c C-s f"]
     ["插入 Wiki Link" markdown-insert-wiki-link
      :keys "C-c C-s w"])
     "---"
     ["插入横线" markdown-insert-hr
      :keys "C-c C-s -"]
     ["插入引言 blockquote" markdown-insert-blockquote]
     ["改为 引用块 blockquote" markdown-blockquote-region]
     ["插入代码块 GFM代码风格" markdown-insert-gfm-code-block]
     ["插入 代码块 pre" markdown-insert-pre]
     ["改为 代码块 pre" markdown-pre-region]
     ["单独页面 编辑代码" markdown-edit-code-block
      :enable (markdown-code-block-at-point-p)]
    "---"
    ("升降和移动 等级"
     ["标记补全" markdown-complete]
     ["删掉当前 码道元素和内容" markdown-kill-thing-at-point]
     "--"
     ["整级上移" markdown-move-up
      :keys "C-c <up>"]
     ["整级下移" markdown-move-down
      :keys "C-c <down>"]
     "--"
     ["升 一级" markdown-promote
      :keys "C-c <left>"]
     ["降 一级" markdown-demote
      :keys "C-c <right>"]
     "--"
     ["增缩进" markdown-indent-region]
     ["减缩进" markdown-outdent-region])
    "---"
    ("选定"
     ["选定一段 Paragraph" mark-paragraph]
     ["选定一块 Block" markdown-mark-block]
     ["选定一节 Section" mark-defun]
     ["选定一章 Subtree" markdown-mark-subtree])
    ("跳转"
     ["Jump" markdown-do]
     ["查看连接" markdown-follow-thing-at-point]
     ["下一连接" markdown-next-link]
     ["上一连接" markdown-previous-link]
     "---"
     ["上一级 不同级/列表" markdown-outline-next]
     ["下一级 不同级/列表" markdown-outline-previous]
     ["下一级 同级" markdown-outline-next-same-level]
     ["前一级 同级" markdown-outline-previous-same-level]
     ["跳到父级" markdown-outline-up]
     "---"
     ["前段 Paragraph" markdown-forward-paragraph]
     ["后段 Paragraph" markdown-backward-paragraph]
     ["前块 Block" markdown-forward-block]
     ["后块 Block" markdown-backward-block])
    "---"
    ("开/关 支持功能"
     ["数学公式支持LaTeX Math" markdown-toggle-math
      :style radio
      :selected markdown-enable-math])

    ("显/隐 文本结构"
     ["显/隐 链接" markdown-toggle-url-hiding
      :style radio
      :selected markdown-hide-urls]
     ["显/隐 Wiki链接" markdown-toggle-wiki-links
      :style radio
     ...
     ["全显" widen (buffer-narrowed-p)]
     "---"
     ["显/隐 标识" markdown-toggle-markup-hiding
      :keys "C-c C-x C-m"
      :style radio
      :selected markdown-hide-markup])
    "---"
    ("文档"
     ["开发者首页" markdown-mode-info]
     ["查看文档" markdown-check-refs]
     ["Find Unused References" markdown-unused-refs]
     ["快捷键一览" (describe-function 'markdown-mode)]
     ["开发者帮助" (browse-url-eww "https://leanpub.com/markdown-mode")]
